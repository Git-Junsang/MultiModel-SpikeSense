...
모델 1개 = 장비 1대(기종 + ID). 전체 41대 중 fan id_00을 제외한 40대를 쓴다.
제외한 장비의 wav는 저장소에서 삭제했다. 처음에는 slider id_06을
임의로 제외했으나, fan id_00이 데이터 자체가 가장 어려운 장비여서(학습 결과 AUC 0.738,
slider id_06은 0.998) 2026-09-23에 둘을 맞바꿨다.
...
DATA_DIR = os.path.join(SOFTWARE_DIR, "data", "dataset_dcase")
CACHE_DIR = os.path.join(SOFTWARE_DIR, "data", "mel_dcase")
...
MACHINE_IDS = {
    "fan":         ["01", "02", "03", "04", "05", "06"],              # id_00 제외
...
def _segments(mel, hop=SEG_FRAMES):
    starts = range(0, len(mel) - SEG_FRAMES + 1, hop)
    return np.stack([mel[s:s + SEG_FRAMES] for s in starts])


def load_unit(machine, mid, split, workers=10, hop=SEG_FRAMES):
    """장비 1대의 세그먼트 캐시를 읽거나 만든다.

    hop < SEG_FRAMES면 구간을 겹쳐 자른다. 학습 표본을 늘리는 용도이며, 평가는
    겹치지 않는 기본 분할(hop = SEG_FRAMES)로 한다.

    반환 dict: seg [S, 31, 40] float32, label [S], file_idx [S](파일 번호), files [F]
    """
    os.makedirs(CACHE_DIR, exist_ok=True)
    suffix = f"_seg{SEG_FRAMES}" + (f"_hop{hop}" if hop != SEG_FRAMES else "")
    cache = os.path.join(CACHE_DIR, f"{machine}_id{mid}_{split}{suffix}.npz")
